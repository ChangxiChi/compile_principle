...
def vardecl(strToken, str_type, indicator):  # 已判断出第一个是var
    ...
    if str_type == '标识符':
        ...
        if t!=-1:
            # var 可以重复定义
            if signal_stack['type']=='const'or signal_stack['type']=='proc':
                print(name+' 这个标识符已经被定义过了')
...
def proc(strToken, str_type, indicator):
    ...
    if str_type == '标识符':
        ...
        para_num += 1
        if strToken == ')':
            fun_para['name'].append(name)
            fun_para['num'].append(para_num)
...
def statment(strToken, str_type, indicator):
    if str_type == "标识符":
        ...
        if strToken == ':=':
            # 注意，const不能被赋值
            if signal_stack['type'][find_name(name)]=='const':
                row, col = find_mistake(indicator)
                print(name+' const不能被赋值'+'('+str(row)+ ','+str(col)+')')
                strToken, str_type, indicator = load_word(word, indicator)
                ...
                if strToken==';':
                    ...
                    gen('sto',str(int(signal_stack['level'])-int(level)),str(a+1+3)) # 将exp的结果送回
        else:
            # print("缺少: :=")
            row, col = find_mistake(indicator)
            print("缺少： :=  (", row, ",", col, ")")
            ...
            if strToken == '(':
                ...
                if strToken == ')':
                    ...
                    if strToken == ')':
                        ...
                        a=signal_stack['address'][idx]  # 找到存在符号表的真正地址
                        gen('cal','0',str(a))  # call的目标代码，第三位表示name跳转到的地址
                        if call_num!=fun_para['num'][fun_para['name'].index(name)]:
                            print(name+'函数调用参数有误')
                    else:
                        # print("缺少： )")
                        row, col = find_mistake(indicator)
    ...
    if strToken == '+' or strToken == '-':
        ...
        while strToken == '+' or strToken == '-':
            ...
            if strToken==';' or strToken=='else' or strToken=='end' or strToken==')': #exp的follow集合
                ...
                # 可能漏打了一个aop
                while strToken != '+' and strToken != '-' and strToken != ')' and strToken != ';' and strToken != 'do' and strToken != 'then' and strToken!='else' and strToken!='end' and strToken!=',':
                    # 只是忘记打aop了，后面一个还是term
                    strToken, str_type, indicator = term(strToken, str_type, indicator)
